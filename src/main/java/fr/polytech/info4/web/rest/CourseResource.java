...
public class CourseResource {
    ...
    public List<CourseDTO> getAllCourses(@RequestParam(required = false) String filter) {
        if ("panier-is-null".equals(filter)) {
            log.debug("REST request to get all Courses where panier is null");
            return courseService.findAllWherePanierIsNull();
        }
        if ("compte-is-null".equals(filter)) {
            log.debug("REST request to get all Courses where compte is null");
            return courseService.findAllWhereCompteIsNull();
        }
