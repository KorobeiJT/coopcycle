...
describe('Cooperative e2e test', () => {
  ...
  it('should load Cooperatives', async () => {
    ...
    expect(await cooperativeComponentsPage.getTitle()).to.eq('dmApp.cooperative.home.title');
    await browser.wait(
  ...
  it('should load create Cooperative page', async () => {
    ...
    expect(await cooperativeUpdatePage.getPageTitle()).to.eq('dmApp.cooperative.home.createOrEditLabel');
  ...
  it('should create and save Cooperatives', async () => {
    ...
    await promise.all([cooperativeUpdatePage.setNameInput('name'), cooperativeUpdatePage.setMembersInput('members')]);

    expect(await cooperativeUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await cooperativeUpdatePage.getMembersInput()).to.eq('members', 'Expected Members value to be equals to members');
  ...
  it('should delete last Cooperative', async () => {
    ...
    expect(await cooperativeDeleteDialog.getDialogTitle()).to.eq('dmApp.cooperative.delete.question');
